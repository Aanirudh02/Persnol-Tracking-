...
<body class="h-full bg-gradient-to-br from-sky-50/60 via-slate-50 to-blue-50/40 text-slate-800 dark:bg-slate-950 dark:text-slate-100 antialiased selection:bg-sky-500 selection:text-white transition-colors duration-200">
    ...
    @if (session('success'))
        <script>
            document.addEventListener('DOMContentLoaded', () => window.showToast && window.showToast("{{ session('success') }}", 'success'));
        </script>
    @endif
    @if (session('error'))
        <script>
            document.addEventListener('DOMContentLoaded', () => window.showToast && window.showToast("{{ session('error') }}", 'error'));
        </script>
    @endif
</body>
</html>
