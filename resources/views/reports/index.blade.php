    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-sky-100 shadow-sm">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                ...
                <div class="flex items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-200 text-xs">
                    <a href="{{ route('reports.index', ['preset' => $filter, 'from_date' => $startDate, 'to_date' => $endDate, 'expense_type' => 'normal']) }}"
                        class="px-3 py-1.5 rounded-lg font-bold transition flex items-center gap-1.5 {{ ($expenseType ?? 'normal') === 'normal' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">
                        <span>📘</span> Normal (Default)
                    </a>
                    <a href="{{ route('reports.index', ['preset' => $filter, 'from_date' => $startDate, 'to_date' => $endDate, 'expense_type' => 'personal']) }}"
                        class="px-3 py-1.5 rounded-lg font-bold transition flex items-center gap-1.5 {{ ($expenseType ?? 'normal') === 'personal' ? 'bg-pink-600 text-white shadow-sm' : 'text-slate-600 hover:text-pink-600' }}">
                        <span>🛍️</span> Personal
                    </a>
                </div>

                <a
                    href="{{ route('reports.export', ['preset' => $filter, 'from_date' => $startDate, 'to_date' => $endDate, 'expense_type' => $expenseType ?? 'normal']) }}"
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
            ...
            @foreach($presets as $key => $label)
                <a
                    href="{{ route('reports.index', ['preset' => $key, 'expense_type' => $expenseType ?? 'normal']) }}"
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-5 border border-sky-100 shadow-sm relative overflow-hidden">
                <div class="flex items-center justify-between text-xs text-slate-500 mb-1">
                    <span class="font-medium">Total {{ ($expenseType ?? 'normal') === 'personal' ? 'Personal Expenses' : 'Expenses' }}</span>
                    <span class="p-1.5 rounded-lg {{ ($expenseType ?? 'normal') === 'personal' ? 'bg-pink-50 text-pink-600' : 'bg-rose-50 text-rose-600' }} font-bold">💳</span>
                </div>
                <div class="text-xl sm:text-2xl font-bold {{ ($expenseType ?? 'normal') === 'personal' ? 'text-pink-600' : 'text-rose-600' }}">₹{{ number_format($totalExpenses, 2) }}</div>
            ...
            <div class="bg-white rounded-2xl border border-sky-100 shadow-sm overflow-hidden">
                <div class="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="text-base">{{ ($expenseType ?? 'normal') === 'personal' ? '🛍️' : '💳' }}</span>
                        <h3 class="font-bold text-sm text-slate-900">{{ ($expenseType ?? 'normal') === 'personal' ? 'Personal Expenses Log' : 'Expenses Log' }}</h3>
                        <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ ($expenseType ?? 'normal') === 'personal' ? 'bg-pink-50 text-pink-700' : 'bg-sky-50 text-sky-700' }}">{{ count($expenses) }}</span>
                    </div>
                </div>
